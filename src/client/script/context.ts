...
export async function runScript(fn: ScenarioFn, ctx: ScriptContext): Promise<ScriptResult> {
  ...
  try {
    ...
    // so they don't fire after the script ends. Idempotent.
    internal._suiAutoResponder.detach();
    // Detach the NPC dialog tracker + clear lastDialog so consumers don't
    // see stale state if they hang onto the ctx reference. Idempotent.
