...
describe.skipIf(!LIVE)('live ctx.bank + ctx.inventory.slots + ctx.datapad.vehicle.condition', () => {
  it('drives the container-ecosystem APIs end-to-end against a real server', async () => {
    ...
    const lifecycleResult = await client.fullLifecycle({
      ...
      script: async (ctx) => {
        ...
        const cmd = `object create ${VEHICLE_CREATURE_TEMPLATE} ` +
        ...
        console.warn(`[live-bank] admin-spawned vehicle creature id=${observed.vehicleId.toString()}`);
    ...
    expect(observed.inventoryUsedSlots, 'usedSlots is a non-negative count').toBeGreaterThanOrEqual(
      0,
    );
