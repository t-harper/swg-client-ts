...
/**
 * A scripting hook forwarded to `SwgClient`. Kept as `unknown` here because
 * the actual `ScriptContext` is defined in the scripting layer that's
 * orthogonal to Fleet. Fleet just forwards the function reference unchanged.
 *
 * Forward-compat: when `FullLifecycleOptions` grows a `script?: ScenarioFn`
 * field, Fleet will pass this through verbatim. Until then it's stored on
 * the config and ignored on dispatch — see `forwardToLifecycle` below.
 */
export type ScenarioFn = (ctx: unknown) => Promise<void>;
...
/**
 * Strip Fleet-only fields and forward to `SwgClient.fullLifecycle`.
 *
 * `script` is a forward-compatible field — when `FullLifecycleOptions` adopts
 * it, change this function to pass it through. Today it's intentionally
 * dropped to keep `tsc --noEmit` clean.
 */
function forwardToLifecycle(config: FleetClientConfig): FullLifecycleOptions {
  const {
    script: _ignoredScript,
    account,
    characterName,
    password,
    clusterName,
  ...
  void _ignoredScript;
  const out: FullLifecycleOptions = { account };
  if (characterName !== undefined) out.characterName = characterName;
  if (password !== undefined) out.password = password;
  if (clusterName !== undefined) out.clusterName = clusterName;
  if (planet !== undefined) out.planet = planet;
  if (profession !== undefined) out.profession = profession;
  if (holdZonedInMs !== undefined) out.holdZonedInMs = holdZonedInMs;
  if (skipGameStage !== undefined) out.skipGameStage = skipGameStage;
  return out;
}
