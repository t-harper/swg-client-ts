...
pnpm install
pnpm test              # ~1756 unit tests — no server needed
LIVE=1 pnpm test       # ~1782 total under LIVE (includes ~26 integration tests against 10.254.0.253)
pnpm test              # ~1740 unit tests — no server needed
...
| **Always-on character sheet** | `ctx.character.health` / `.cashBalance` / `.bankBalance` / `.skills` / `.posture` / `.level` / `.guildId` / `.skillTitle` / `.playedTime` / `.position` / `.group` etc. | Live view of player CREO + PLAY baselines, kept current by the dispatcher loop. Subscribes to `BaselinesMessage` / `DeltasMessage` / `ObjControllerMessage(CM_setPosture=305)`. `ready` flips true after first CREO baseline. Detached automatically at logout. |
| **Always-on group view** | `ctx.group.id` / `.size` / `.leader` / `.members` / `follow(leaderId)` | Live join of `m_group` (CREO) with the GroupObject's SHARED_NP baseline (`GroupObjectSharedNpDecoder` — new). Each `GroupMember` carries `{id, name, position, health, posture, distance}` resolved against the WorldModel so reads are always current. `follow(leaderId)` subscribes to the leader's `UpdateTransformMessage` / `UpdateTransformWithParentMessage` broadcasts and re-emits them as our own `CM_netUpdateTransform=113` (Fleet "everyone follow the mayor" pattern); returns an unsubscribe fn. Auto-detached at `runScript` cleanup. |
| **Always-on guild view** | `ctx.guild.id` / `.name` / `.abbrev` / `.rank` / `.members` | Numeric `guildId` from CREO; `name` / `rank` / `members` typically stay `null`/empty on a single-server cluster because the GuildObject's per-guild detail lives in the SERVER-only package and isn't broadcast to regular clients. `abbrev` surfaces best-effort from `m_abbrevs` (SHARED) when exactly one is present. Use `ctx.guild.id !== 0` as the actionable signal. |
| **Chat-handler subscriptions** | `ctx.chat.onSay(predicate, handler)` / `.onTell(...)` / `.onSystemMessage(...)` | Predicate-driven subscriptions to inbound chat. `predicate` is a `RegExp` (matched against text) OR `(text, sender) => boolean`. `onSay` fires for `ObjControllerMessage(CM_spatialChatReceive=244)` with sender NetworkId + name; `onTell` fires for `ChatInstantMessageToClient` (sender name only — ChatAvatarId not NetworkId-keyed); `onSystemMessage` fires for `ChatSystemMessage` (no sender). Each returns an unsubscribe fn; all per-script subscriptions are auto-detached at `runScript` cleanup. |
| **Always-on inventory** | `ctx.inventory.items` / `findByTemplate(re)` / `findById(id)` / `ready` / `containerId` / **`totalSlots` / `usedSlots` / `freeSlots`** / **`resources()`** | Auto-opened at zone-in via `ClientOpenContainerMessage(playerId, 'inventory')`. Items derived live from WorldModel — no manual openContainer or transcript walking. 3-tier discovery (template-name → SHARED-baseline `nameStringId={item_n, inventory}` → player-child heuristic) because the live server pushes containers via ByCrc, not ByName. `totalSlots` defaults to 80 (matching the SHARED template asset's `containerVolumeLimit`; override via `setTotalSlots`); `usedSlots = items.length`; `freeSlots = max(0, total - used)`. `resources()` filters items to RCNO crates and surfaces `{containerId, resourceType, quantity}` from each crate's RCNO SHARED baseline. |
| **Always-on datapad** | `ctx.datapad.items` / `vehicles()` / `pets()` / `waypoints()` / `missions()` / `findByTemplate(re)` plus **`vehicles()[i].condition / .state / .linkedCreatureId`** | Auto-opened at zone-in via `ClientOpenContainerMessage(playerId, 'datapad')`. Items classified by templateName into vehicle-pcd / pet-pcd / waypoint / mission / ship / manufacturing-schematic / other. PCDs additionally carry `condition` (HP ratio of the linked live creature in [0,1], derived from its TANO SHARED `damageTaken`/`maxHitPoints`) + `state` (`'stored'/'called'/'following'/'staying'/'attacking'` — driven by `notifyPetAction` hooks wired into `callVehicle`/`storeVehicle`/`callPet`/`storePet`/`petCommand`). Linkage to the live creature happens automatically when a CREO SHARED baseline arrives with `masterId === playerNetworkId` after a pending `call*` action. Used by `rideVehicleScenario` for zero-config PCD lookup. |
| **Always-on bank** | `ctx.bank.items` / `findByTemplate(re)` / `findById(id)` / `ready` / `containerId` / **`use(terminalId?)`** | Bank container is a per-player slot (`SlotNames::bank`); auto-discovered via template-name match or SHARED-baseline `nameStringId={item_n, bank}`. NOT auto-opened at zone-in — contents only stream after `ctx.bank.use(terminalId)` fires `ObjectMenuSelectMessage(terminalId, ITEM_USE=21)` on a bank terminal (or after a direct `ClientOpenContainerMessage(playerId, 'bank')` send — the same wire side-effect `openBankContainer` produces server-side). When `terminalId` is omitted, `use()` scans the WorldModel for the nearest object whose template matches `/bank_terminal|terminal_bank/i` and uses that; throws if none in range. |
...
| **Character enriched views** | `ctx.character.skillMods` (CREO p4 `m_modMap` → `Map<name, base+bonus>`) / `.xp` (PLAY p8 `m_experiencePoints`) / `.effects` (CREO p6 `m_buffs` → `{name, magnitude, durationSec, expiresAt}[]`) / `.weapon` (CREO p6 `m_currentWeapon` joined with WEAO baselines + cached AttributeListMessage → `{networkId, templateName, minDamage, maxDamage, attackSpeed, range, ammoRemaining}` ‖ `null`) / `.roadmap` (PLAY p8 `m_workingSkill` + `m_activeQuests` → `{currentPhase, currentTask, tasksRemaining}` ‖ `null`) / `.factionDetails` (`{type, name='neutral'\|'imperial'\|'rebel', standing, pvpStatus}`) | Derived live views over the new baselines: **CREO p4** (`creature-object-baseline-4.ts` — 16 fields with `m_modMap`/`m_attribBonus`/`m_runSpeed`/etc.), **PLAY p8** (`player-object-baseline-8.ts` — 9 fields with `m_experiencePoints`/`m_workingSkill`/`m_waypoints`/quest book), **WEAO p3** (`weapon-object-baseline-3.ts` — attackSpeed/accuracy/minRange/maxRange/damageType/elementalType), **WEAO p6** (`weapon-object-baseline-6.ts` — weaponType). All four packages have matching delta decoders so the views stay current. `AttributeListMessage` is also subscribed so `weapon.minDamage`/`maxDamage`/`ammoRemaining` populate after `ctx.fetchResourceAttributes([weaponId])` (those fields are server-only baselines — `getAttributesBatch` is the only client-facing source). |
...
| **SUI dialogs** | `ctx.waitForSui()` / `ctx.respondToSui(pageId, eventType, returnList?)` plus **`ctx.sui.autoRespond(predicate, 'ok'|'cancel'|{eventType,returnList})`** + **`ctx.sui.active`** | Receive server-pushed SUI pages (`SuiCreatePageMessage` / `SuiUpdatePageMessage` / `SuiForceClosePage`) and reply with `SuiEventNotification`. Page widget tree decoded (`SuiPageData` with 9 typed `SuiCommand` variants). High-level `ctx.sui.autoRespond` registers standing handlers that fire `SuiEventNotification` automatically on matching pages (predicate inspects the full `SuiPageData`); first match wins; auto-detached during `runScript` cleanup. `ctx.sui.active` lists every currently-displayed page with its best-effort `title` (extracted from `setProperty` commands). |
| **NPC conversation** | `ctx.talkTo(npcId)` / `ctx.waitForNpcDialog()` / `ctx.selectDialog(n)` / `ctx.endConversation()` plus **`ctx.npc.converse(npcId, path)`** + **`ctx.npc.lastDialog`** | Start/respond/stop handshake via command-queue. `waitForNpcDialog` pairs `CM_npcConversationMessage(223)` + `CM_npcConversationResponses(224)`. High-level `ctx.npc.converse(npcId, ['greet', 'inquire']|[0, 1])` walks a known dialog tree end-to-end (substring match on labels OR numeric indices). Throws if any step fails to match an available option. `ctx.npc.lastDialog` is a passive cache of the most-recent `{text, options}` pair, cleared on `CM_npcConversationStop`. |
...
1. `cd ~/code/swg-ts-client && nvm use && pnpm test` — confirm baseline (should be ~1756 unit green; ~1782 total under `LIVE=1`).
1. `cd ~/code/swg-ts-client && nvm use && pnpm test` — confirm baseline (should be ~1740 unit green; ~1770 total under `LIVE=1`).
<<<<<<< HEAD
