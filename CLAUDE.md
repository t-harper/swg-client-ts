...
- **Spatial chat (`/say`, `/shout`) is not yet wire-decoded.** It's an `ObjControllerMessage` subtype (`MessageQueueSpatialChat`). `ctx.say()` currently routes through tell-to-self as a placeholder that exercises the chat pipeline.
