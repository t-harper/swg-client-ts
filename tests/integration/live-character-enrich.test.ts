...
describe.skipIf(!LIVE)('live character-sheet enriched views', () => {
  it('populates skillMods, xp, effects, weapon, roadmap, factionDetails from real wire data', async () => {
    ...
    const lifecycleResult = await client.fullLifecycle({
      ...
      script: async (ctx) => {
        ...
        observed.roadmap = ctx.character.roadmap as unknown as Record<string, unknown> | null;
    ...
    expect(w.range, 'weapon.range > 0 (from WEAO p3 m_maxRange)').toBeGreaterThan(0);
