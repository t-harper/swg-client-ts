...
describe('walkToCell', () => {
  it('emits CM_netUpdateTransformWithParent (id 241), not the world variant', async () => {
    ...
    await ctx.walkToCell(CELL_ID, { x: 0, z: 10 }, { tickMs: 200 });

    const moves = cellMovementSends(sent);
    // 10m walk @ locked BASE_RUN_SPEED=7.3m/s @ 200ms tick → ~7 ticks
  ...
  it('sets parentCell on every send to the supplied parent', async () => {
    const { ctx, sent } = createFakeContext();
    await ctx.walkToCell(CELL_ID, { x: 5, z: 0 }, { tickMs: 200 });
  ...
  it('preserves exact cell-relative floats (no fixed-point quantization)', async () => {
    ...
    await ctx.walkToCell(CELL_ID, { x: 3.5, z: 1.25 });
    const moves = cellMovementSends(sent);
  ...
  it('sends with the player networkId from sceneStart in the ObjController header', async () => {
    ...
    await ctx.walkToCell(CELL_ID, { x: 1, z: 1 }, { tickMs: 200 });
  ...
  it('updates ctx.cellPosition() / ctx.parentCell() as it moves', async () => {
    ...
    await ctx.walkToCell(CELL_ID, { x: 10, z: 0 }, { tickMs: 200 });
  ...
  it('resets cell cursor to (0,0,0) when entering a new cell', async () => {
    ...
    await ctx.walkToCell(OTHER_CELL, { x: 5, z: 0 }, { tickMs: 200 });
    const moves = cellMovementSends(sent);
    const first = moves[0];
  ...
  it('honors setCellPose for the starting cursor when already in the cell', async () => {
    ...
    await ctx.walkToCell(CELL_ID, { x: 5, z: 6 }, { tickMs: 200 });
    const moves = cellMovementSends(sent);
  ...
  it('aborts cleanly when the signal fires mid-walk', async () => {
    ...
    const walkPromise = ctx.walkToCell(CELL_ID, { x: 0, z: 100 }, { tickMs: 100 });
  ...
  it('counts every send in scriptResult.sendsCount (including teleport-ack bootstrap)', async () => {
    const { ctx } = createFakeContext();
    const baselineSendsCount = (ctx as unknown as { _state: { sendsCount: number } })._state
      .sendsCount;
    await ctx.walkToCell(CELL_ID, { x: 0, z: 10 }, { tickMs: 200 });
    const finalSends = (ctx as unknown as { _state: { sendsCount: number } })._state.sendsCount;
    // ~7 movement sends @ locked BASE_RUN_SPEED plus ≥1 teleport-ack send.
    expect(finalSends - baselineSendsCount).toBeGreaterThanOrEqual(7);
  });
});
