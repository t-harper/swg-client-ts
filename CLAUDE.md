...
| **DeltasMessage decoding** | `DeltasMessage` + `deltaRegistry` + `tryDecodeDelta` + per-package decoders under `src/messages/game/baselines/*-delta-*.ts` | Post-baseline incremental updates. Envelope mirrors `BaselinesMessage` (target/typeId/packageId/packageBytes); a registered `(typeId, packageId)` decoder produces `decodedDelta: { kind, data: Partial<BaselineType> }` carrying just the changed fields. Without it, the in-memory world view goes stale at zone-in. Ships with TANO p1 (bank/cash) as a primitive-fields template; add more per-package decoders by mirroring the matching `*-baseline-*.ts` file's `addVariable` order. AutoDelta* container fields require per-container delta codecs (separate format from baseline; not yet implemented — see `AutoDeltaVector/Set/Map.h::packDelta`). |
