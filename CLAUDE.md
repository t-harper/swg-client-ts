...
pnpm install
pnpm test              # ~1776 unit tests — no server needed
LIVE=1 pnpm test       # ~1804 total under LIVE (includes ~29 integration tests against 10.254.0.253)
...
1. `cd ~/code/swg-ts-client && nvm use && pnpm test` — confirm baseline (should be ~1776 unit green; ~1804 total under `LIVE=1`).
