...
| Navigate to the nearest named place (starport / cantina / bank) | `ctx.map.nearest('starport')` / `ctx.map.list('bank')` / `ctx.map.goTo('cantina')` (see `src/client/script/map.ts`). Backed by SWG's planetary-map system: `GetMapLocationsMessage` → `GetMapLocationsResponseMessage` (a `PlanetMapCacheImpl` requests + caches per planet). Planet-wide, unlike `ctx.world`'s awareness-limited scan. Scenario: `goto-nearest` (`--script-arg=category=`). |
