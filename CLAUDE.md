...
LIVE=1 pnpm test       # ~1177 total under LIVE (includes ~25 integration tests against 10.254.0.253)
...
| **SUI dialogs** | `ctx.waitForSui()` / `ctx.respondToSui(pageId, eventType, returnList?)` | Receive server-pushed SUI pages (`SuiCreatePageMessage` / `SuiUpdatePageMessage` / `SuiForceClosePage`) and reply with `SuiEventNotification`. Page widget tree is opaque bytes. |
...
1. `cd ~/code/swg-ts-client && nvm use && pnpm test` — confirm baseline (should be ~1152 unit green; ~1177 total under `LIVE=1`).
