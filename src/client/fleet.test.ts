...
describe('Fleet', () => {
  ...
  it('forwards all config fields except `script` to fullLifecycle', async () => {
    ...
    // `script` is dropped (forward-compat with future scripting layer).
    expect((captured as Record<string, unknown> | undefined)?.script).toBeUndefined();
  });
